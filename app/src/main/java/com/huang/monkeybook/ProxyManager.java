...
public class ProxyManager {
    ...
    public static void saveProxyState(boolean state) {
        proxyState = state;
        SharedPreferences.Editor editor = App.getInstance().getSharedPreferences("CONFIG", 0).edit();
    ...
    private static void initProxyState() {
        try {
            packageName = App.getInstance().getPackageManager().getPackageInfo(App.getInstance().getPackageName(), 0).packageName;
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("=================包名获取失败，可能会影响代理请求功能=================");
        }
        proxyState = App.getInstance().getSharedPreferences("CONFIG", 0).getBoolean(SP_KEY_PROXY_STATE, PROXY_STATE_DEFAULT);
    }
    ...
    private static void initProxyHttp() {
        proxyHttp = App.getInstance().getSharedPreferences("CONFIG", 0).getString(SP_KEY_PROXY_HTTP, PROXY_HTTP_DEFAULT);
    }
